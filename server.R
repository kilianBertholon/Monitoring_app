...
server <- function(input, output, session) {
  ...
  create_plotly_chart <- function(data,
    ...
    yellow_zone_sup <- list(
      ...
      x0 = min(data$Date),
      x1 = max(data$Date),
      y0 = selected_range$range_max,
      y1 = round(selected_range$range_max + (selected_range$range_max * 0.05),2),
    ...
    #   x1 = max(data$Date),
    #   y0 = selected_range$range_min,
    #   y1 = if(selected_range$range_min > 0) {
    #     selected_range$range_min - (selected_range$range_min * 0.05)
    #   } else {
    #     0
    #   },
    #   fillcolor = "rgba(255, 255, 0, 0.5)",
