...
server <- function(input, output, session) {
  ...
  create_plotly_chart <- function(data, variable_name, title, highlight_color) {
    sujet_select <- input$Sujet
    data_sujet <- data[data$Sujet == sujet_select, ]
    plot_ly(data, x = ~Date, y = ~Valeur, type = "scatter", mode = "markers",
            marker = list(color = ~ifelse(data$Sujet == sujet_select, highlight_color, "gray")),
            showlegend = FALSE) %>%
      add_trace(data = data_sujet, type = "scatter", mode = "lines",
...
observe({
  ...
  if (!is.null(sujet_select)) {
    data_sujet <- data_num
    data_sujet$highlight_color <- ifelse(data_sujet$Sujet == sujet_select, "blue", "gray")
    ...
    output$graph_FC_recup <- renderPlotly({
      create_plotly_chart(data_sujet[data_sujet$Variable == "FC_recup",],
                                             "Fréquence cardiaque de récupération (bpm)", "FC récup", "blue")
    })
    ...
    output$Proteine_C_reactive <- renderPlotly({
      create_plotly_chart(data_sujet[data_sujet$Variable == "Proteine_C_reactive",],
                          "Proteine C reactive", "Proteine C reactive", "blue")
    })
    ...
    output$Sat_transferrine <- renderPlotly({
      create_plotly_chart(data_sujet[data_sujet$Variable == "Sat_transferrine",],
                          "Saturation en transferrine", "Saturation en transferrine", "blue")
    })
    ...
    output$e1_25_dihydroxyvitamine_D <- renderPlotly({
      create_plotly_chart(data_sujet[data_sujet$Variable == "1_25-dihydroxyvitamine_D",],
                          "1_25_dihydroxyvitamine_D", "1_25_dihydroxyvitamine_D", "blue")
    })
